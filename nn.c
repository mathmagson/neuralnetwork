...
main() {
    ...
    double Error, eta = 0.45, alpha = 0.9, smallwt = 0.5;
...
/*              DeltaO[k] = Target[p][k] - Output[p][k];     Linear Outputs, SSE */
    ...
    for( j = 1 ; j <= NumHidden ; j++ ) {    /* compute hidden unit activations */
        SumH[4][j] = WeightIH[0][j] ;
        for( i = 1 ; i <= NumInput ; i++ ) {
            SumH[4][j] += Input[4][i] * WeightIH[i][j] ;
        }
        Hidden[4][j] = 1.0/(1.0 + exp(-SumH[4][j])) ;
    }

    for( k = 1 ; k <= NumOutput ; k++ ) {    /* compute output unit activations and errors */
        SumO[4][k] = WeightHO[0][k] ;
        for( j = 1 ; j <= NumHidden ; j++ ) {
            SumO[4][k] += Hidden[4][j] * WeightHO[j][k] ;
        }
        Output[4][k] = 1.0/(1.0 + exp(-SumO[4][k])) ;   /* Sigmoidal Outputs */
        fprintf(stdout, "\nInputs %d XOR %d = %f\n", (int)Input[4][1], (int)Input[4][2], Output[4][k]);
    }
